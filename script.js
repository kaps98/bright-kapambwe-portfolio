...
const contactCards = document.querySelector("[data-contact-cards]");

function createStatCards() {
  statStrip.innerHTML = portfolioData.stats
    .map(
      (item) => `
        <article class="stat-card">
          <span class="stat-value" data-target="${item.value}" data-suffix="${item.suffix}">0${item.suffix}</span>
...
function createSignalCards() {
  signalGrid.innerHTML = portfolioData.focusAreas
    .map(
      (item) => `
...
function createSocialArea() {
  ...
  if (portfolioData.profile.linkedin) {
    links.push(
      `<a class="pill" href="${portfolioData.profile.linkedin}" target="_blank" rel="noreferrer">LinkedIn</a>`
    );
  }
...
function renderFilters(active = "All") {
  ...
  filterRow.innerHTML = portfolioData.filters
    .map(
      (filter) => `
        <button class="filter-button ${filter === active ? "is-active" : ""}" type="button" data-filter="${filter}">
          ${filter}
        </button>
      `
    )
...
function renderProjects(active = "All") {
  const projects =
    active === "All"
      ...
      : portfolioData.projects.filter((project) => project.category === active);
...
function renderPanels(target, title, items) {
  target.innerHTML = `
    <article class="panel" data-reveal>
      <div class="section-heading section-heading--compact">
        <p class="eyebrow">${title}</p>
        <h2>${title === "Education" ? "Academic Foundation" : "Certification Track"}</h2>
      </div>
            ...
            (item) => `
              <article class="panel">
                <div class="panel-header">
                  <div>
                    <h3>${item.title}</h3>
                    <div class="panel-subtitle">${item.institution}</div>
                  </div>
                  <span class="panel-label">${item.label}</span>
                </div>
                <div class="panel-subtitle">${item.period}</div>
                <p>${item.summary}</p>
              </article>
            `
          )
...
function renderSkills() {
  skillGroups.innerHTML = portfolioData.skillGroups
    .map(
      (group) => `
...
function animateCounters() {
  ...
  values.forEach((value) => statObserver.observe(value));
}
